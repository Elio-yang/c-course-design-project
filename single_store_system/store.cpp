...
void add_record_file(RecordList* RL,MaterialList* ML){ //!从文件读取进货信息
        printf(GREEN "<<<<<<<<<<测试接口：读取进货记录   测试数据:文件\n" NONE);
        ...
        while(fscanf(file,"%s %s %s %d",Node->time,Node->material_name,Node->wholesaler_name,&Node->num)!=EOF){
                ...
                int idx = Node->material_name[5]-'a';
                update_finance_record(Node->material_name,src[idx],INPUT);
        }
...
void change_time_record_material(RecordList* RL,MaterialList* ML,char time[30],int new_num){
        printf(GREEN "<<<<<<<<<<测试接口：按照时间和改后数量修改进货记录   测试数据:%s %d\n" NONE,time,new_num);
...
void query_record_time_to_time(RecordList* RL,char time1[30],char time2[30]){
        printf(GREEN "<<<<<<<<<<测试接口：按照时间范围查询进货记录   测试数据:%s到%s\n" NONE,time1,time2);
        if(RL->RecordHead.next==NULL) {printf("进货记录为空"); return;}
        RecordNode* temp=RL->RecordHead.next;
        int cnt=1;
        int flag=0;
...
void query_record_material_name(RecordList* RL,char material_name[30]){
        printf(GREEN "<<<<<<<<<<测试接口：按照原料查询进货记录   测试数据:%s\n" NONE,material_name);
        if(RL->RecordHead.next==NULL) {printf("进货记录为空"); return;}
        RecordNode* temp=RL->RecordHead.next;
        int cnt=1;
        int flag=0;
...
void query_record_wholesaler_name(RecordList* RL,char wholesaler_name[30]){
        printf(GREEN "<<<<<<<<<<测试接口：按照处理人查询进货记录   测试数据:%s\n" NONE,wholesaler_name);
        if(RL->RecordHead.next==NULL) {printf("进货记录为空"); return;}
        RecordNode* temp=RL->RecordHead.next;
        int cnt=1;
        int flag=0;
...
void sort_record_time(RecordList* RL){
        printf(GREEN "<<<<<<<<<<测试接口：按照时间排序进货记录   测试数据：无\n" NONE);
        if(RL->RecordHead.next==NULL) {printf("进货记录为空"); return;}
        printf("以时间排序为:\n");
...
void sort_record_material_name(RecordList* RL){
        printf(GREEN "<<<<<<<<<<测试接口：按照货物名称排序进货记录   测试数据：无\n" NONE);
        if(RL->RecordHead.next==NULL) {printf("进货记录为空"); return;}
        printf("以货物名称排序为:\n");
        RecordNode* head=(RecordNode*)malloc(sizeof(RecordNode));
        head->next=NULL;
...
void sort_record_wholesaler_name(RecordList* RL){
        printf(GREEN "<<<<<<<<<<测试接口：按照处理人排序进货记录   测试数据：无\n" NONE);
        if(RL->RecordHead.next==NULL) {printf("进货记录为空"); return;}
        printf("以处理人排序为:\n");
        RecordNode* head=(RecordNode*)malloc(sizeof(RecordNode));
        head->next=NULL;
...
int available_num(char type[30],char goods[30],GoodList* GL,MaterialList* ML){
        printf(GREEN "<<<<<<<<<<测试接口：查询商品可以做数量   测试数据：%s,%s\n" NONE ,type,goods);
        if(strcmp(type,"Wine")==0){
                ...
                if(i==GL->wine_num) {printf("没有这个商品！"); return -1;}
        ...
        if(strcmp(type,"Snacks")==0){
                ...
                if(i==GL->snacks_num) {printf("没有这个商品 亲！"); return -1;}
        ...
        if(strcmp(type,"Tea")==0){
                ...
                if(i==GL->tea_num) {printf("没有这个商品 亲！"); return -1;}
        ...
        if(strcmp(type,"Drinks")==0){
                ...
                if(i==GL->drinks_num) {printf("没有这个商品 亲！"); return -1;}
...
void order_material(char type[30],char goods[30],GoodList* GL,MaterialList* ML,int count){
        printf(GREEN "<<<<<<<<<<测试接口：点单并更新仓库   测试数据：%s,%s,%d\n" NONE,type,goods,count);
        if(strcmp(type,"Wine")==0){
                //!找到WineArr对应index
                int i;
                for(i=0;i<GL->wine_num;i++){
                ...
                if(i==GL->wine_num) {printf("没有这个商品!\n"); return;}
        ...
        if(strcmp(type,"Snacks")==0){
                ...
                if(i==GL->snacks_num) {printf("没有这个商品!\n"); return;}
        ...
        if(strcmp(type,"Drinks")==0){
                ...
                if(i==GL->drinks_num) {printf("没有这个商品!\n"); return;}
...
void print_material(MaterialList* ML){
        printf(GREEN "<<<<<<<<<<测试接口：查询原料   测试数据：无\n" NONE);
        int i;
        for(i=0;i<ML->num;i++){
                printf("第%d种原料: ",i+1);
                printf("名称:%s ",ML->MaterialArr[i].name);
                printf("数量:%d ",ML->MaterialArr[i].num);
                printf("\n");
        }
}